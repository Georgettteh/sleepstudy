...
model1 <- lm(t_mean~ altitude +lat + lon, data = climfr)
...
# Intercept: The mean temperature is 37.27 when altitude, 
# latitude and longitude areall zero.
# 
# Altitude: A one-unit increase in altitude is associated with a temperature decrease of 0.00641 units.
# (b= 0.0064,SE = 0.00087, p<0.001)
# Latitude: A one-unit increase in latitude is linked to a temperature decrease of 0.534 units.
# #(c= 0.534,SE = 0.0557, p<0.001)
# Longitude: The effect of longitude on temperature is not significant (p-value = 0.424).
...
#Longitude (lon): The coefficient for the variable
#"lon" is accompanied by a p-value of 0.424.

model2 <- lm(t_mean~ altitude + lat, data = climfr)
summary(model2)
...
prediction_mont_ven <- model2$coefficients[1] + model2$coefficients[2]*(1212) + model2$coefficients[3]*(44.16)
prediction_mont_ven

prediction_pic_dm <- 37.9147 + model2$coefficients[2]*(2860) + model2$coefficients[3]*(42.93)
prediction_pic_dm
...
# The predicted annual mean  temperature of Mont-Ventoux is 6.17 with a 95%
...
scatter_3d <- with(climfr, scatterplot3d(altitude, lat, t_mean,
                                           pch = 16, highlight.3d = TRUE, angle = 45))
scatter_3d$plane3d(model2)

# The intercept represent the mean annual temperature of 37.915 when altitude, 
# latitude and longitude are zero. Thus  (a= 37.915, SE = 2.483, p<0.001)

# Altitude is negatively associated with mean annual temperature, Thus every 
# increase in altitude by one unit the average mean annual temperature 
# decreases by 0.00636 units. Thus (b= 0.00364,SE = 0.000844, p<0.001)
...
# decreases by 0.547 units. Thus(c= 0.547,SE = 0.0533, p<0.001)

# –	The R-squared value of 83% indicates that 83% of the variance 
#in the mean annual temperature can be explained by altitude and latitude
